...
<h1 class="mt-4">Hiệu Chỉnh : Hợp Đồng Đầu Tư</h1>
        ...
        <div class="form-row align-items-center">
          ...
          <div class="col col-3">
            ...
            <select class="custom-select mr-sm-2" id="inlineFormCustomSelect" name="sltCDT">
              @foreach($data as $row)
                <option hidden value="{{$row->tencdt}}">{{$row->tencdt}}</option>   
              @endforeach
      ...
      <div class="form-group form">
        <div class="form-row">
          ...
          <div class="col">
            ...
            <input type="date" class="form-control" value="@foreach($data as $row) {{$row->ngaylaphddt}} @endforeach" id="txtNLap" name="txtNLap">
          </div>
        </div>
      </div>
      <div class="form-group form">
        <div class="form-row">
          ...
          <div class="col">
            ...
            <input type="text" class="form-control" placeholder="Nhập số" value="@foreach($data as $row) {{$row->tongvon}} @endforeach" id="txtTVon" name="txtTVon">
          </div>
          <div class="col">
              <label for="InputName">Thời Hạn</label>
            <input type="text" class="form-control" placeholder="Nhập số năm" value="@foreach($data as $row) {{$row->thoihan}} @endforeach" id="txtTHan" name="txtTHan">
          </div>
        </div>
      </div>
      <div class="form-group form">
